...
- The first viewport is the journey itself: a cinematic Streckenplan of seven real approaches to the valley — Zürich; Bern, Basel, Genf, St. Gallen (joining at Chur via Sargans) and Luzern (via Zug) through the true rail network; and Lugano arriving from the south over Tirano, up the valley past Brusio. The camera pulls back at Poschiavo, and the 15-minute walking trail runs to the bottom edge of the map, ending at a large home icon standing directly in front of the page's own "Casa Maranta" post title — the icon receives each arrival with a single pop. No caption, no duplicate name, no decorative vignettes.
...
Spacing derives nominally from one governing constant, `--plate: 58px` — the height of a signpost plate. From it come the rhythm tokens: the gap between stacked plates (`--gap`, ~9px), the standard edge and inner padding (`--pad`, ~20px), the separation between sections (`--void`, ~42px), the depth of a plate's point (`--point`, ~18px), the top bar (`--bar`, ~56px), and the bottom nav (`--nav`, ~66px). Fixed bars that sit against a device edge use the safe-area twin instead — `--nav-safe` adds `env(safe-area-inset-bottom)` on top of the bar height, so the bar *grows* by the home-indicator inset rather than absorbing it. Be clear about what this does and does not mean: the constant governs the layout rhythm — plate height, stack gaps, page gutters, section breaks, bar heights — while roughly forty component-level values (icon sizes, checklist paddings, QR dimensions, individual type sizes) are still hard-coded px. Derive new *layout* measurements from `--plate`; do not claim the whole system is a derived scale.
...
- **Two authored moments, both arrival stories:**
  ...
  - **The journey** — the home screen's Streckenplan draws itself over ~8s: the route paints in signal yellow behind a travelling train dot that eases out of and into every station (per-segment ease-in-out, duration ∝ length^0.75), stations ignite as bolts with a 1.6× pop, real altitudes fade in at the stations that tell the climb, the Ospizio Bernina summit triangle lights at 2253 m, and arrival dissolves into a dashed walking trail (slow 2.6s dash-march) that points at the post title below — the page's own "Casa Maranta" is the endpoint, at which the LOSLEGEN plate gives a single 0.55s lift on the first run. A camera rides close beside the train (zoom 2.4, exponential chase), flies to each origin between journeys, and pulls back over 1.3s at Poschiavo so arrival doubles as the reveal of the whole network. Seven origins cycle: Zürich; Bern, Basel and Genf via Lausanne–Fribourg–Olten–Aarau; St. Gallen via Sargans, joining the main line at Chur (the ridden portion is revealed with a dash-pattern window, never from Zürich); Luzern via Zug; and Lugano riding the southern Bernina approach up the valley — the one journey where the main line stays dark and only the south lights. On each arrival the home icon in front of the post title pops once (scale 1.22 on --settle). ~4.6s overview rest between journeys. Landmark vignettes were tried and removed — the map stays pure signage; do not reintroduce decorative POI icons. Map text is painted with fill alone (`#journey-svg text { stroke: none }`): the global svg stroke rule must never touch glyphs. The cycle stops when the guest leaves the home screen and restarts from Zürich on return; `prefers-reduced-motion` renders the completed Zürich map statically.
